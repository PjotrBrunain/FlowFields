...
{	
	...
	auto steeringBehavior = m_pAgentToEvade->GetSteeringBehavior();
	SAFE_DELETE(steeringBehavior);
...
void App_Flocking::Start()
{
	...
	pWanderBehavior->SetWanderOffset(0);
	pWanderBehavior->SetMaxAngleChange(Elite::ToRadians(90));
	m_pAgentToEvade->SetSteeringBehavior(pWanderBehavior);
