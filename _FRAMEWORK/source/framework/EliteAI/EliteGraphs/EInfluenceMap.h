...
namespace Elite
{
	...
	template <class T_GraphType>
	inline void InfluenceMap<T_GraphType>::PropagateInfluence(float deltaTime)
	{
		// TODO: implement
		...
		if (m_TimeSinceLastPropagation >= m_PropagationInterval)
		{
			for (auto* pNode : GetAllActiveNodes())
			{
				...
				{
					float cost{ GetNode(pConnection->GetTo())->GetInfluence() * expf(-pConnection->GetCost() * m_Decay) };
					if (abs(cost) > abs(highestCost)) 
						highestCost = cost;
				}
				float actualCost = Lerp(highestCost, pNode->GetInfluence(), m_Momentum);
				m_InfluenceDoubleBuffer[pNode->GetIndex()] = actualCost;
			}
