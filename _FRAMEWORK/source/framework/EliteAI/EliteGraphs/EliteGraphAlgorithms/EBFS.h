...
namespace Elite 
{
	...
	std::vector<T_NodeType*> BFS<T_NodeType, T_ConnectionType>::FindPath(T_NodeType* pStartNode, T_NodeType* pDestinationNode)
	{
		...
		std::map<T_NodeType*, T_NodeType*> closedList; //checked nodes AND history of made connections
		...
		while (!openList.empty())
		{
			...
			for (T_ConnectionType* pConnection : m_pGraph->GetNodeConnections(pCurrentNode))
			{
